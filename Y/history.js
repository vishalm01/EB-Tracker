import { collection, getDocs, query, orderBy, deleteDoc, doc, updateDoc } from "https://www.gstatic.com/firebasejs/9.15.0/firebase-firestore.js";
...
function calculateBill(units) {
  ...
  if (units <= 100) {
    ...
    totalCost = 300 * 4.7 + 100 * 6.3 + 100 * 8.4 + 200 * 9.45 + (units - 800) * 10.5;
  } else {
    totalCost = 300 * 4.7 + 100 * 6.3 + 100 * 8.4 + 200 * 9.45 + 200 * 10.5 + (units - 1000) * 11.55;
  }
...
async function populateReadingsTable() {
  ...
  if (readings.length === 0) {
    readingsTableBody.innerHTML = "<tr><td colspan='6'>No readings available</td></tr>";
    return;
  }
    ...
    .map((reading, index) => {
      ...
      return `
        <tr id="row-${reading.id}">
          ...
          <td>
            <button class="btn btn-primary btn-md" onclick="editReading('${reading.id}', '${reading.value}')">
              <i class="fas fa-pencil-alt"></i>
            </button>
            <button class="btn btn-danger btn-md m-1" onclick="deleteReading('${reading.id}')">
...
async function editReading(readingId, oldValue) {
  ...
  if (newValue === null || isNaN(parseFloat(newValue)) || parseFloat(newValue) <= 0) {
